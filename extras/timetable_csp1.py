...
class TimetableCSP:
    ...
    4. No free periods between consecutive classes
    """
    ...
    def __init__(self, db: Database, selected_batches: List[int],
        ...
        self.fixed_assignments_set = set()  # Track fixed assignments to preserve them
    ...
    def _check_slot_type_constraint(self, assignment: Dict, slot: Tuple[str, str], 
        ...
        """
        NEW CONSTRAINT: Check if slot division already has a different type assigned
    ...
    def _check_consecutive_free_period(self, assignment: Dict, slot: Tuple[str, str],
                                        current_solution: List[Dict]) -> bool:
        """
        NEW CONSTRAINT: Check if adding this creates a free period between classes
    ...
    def _check_hard_constraints(self, assignment: Dict, slot: Tuple[str, str], 
        ...
        # 5. NEW: Slot type constraint (one type per division)
        if self._check_slot_type_constraint(assignment, slot, current_solution):
            return True

        # 6. NEW: No free periods between consecutive classes
    ...
    def run(self, progress_callback: Optional[Callable] = None) -> Tuple[List[Dict], List[Dict]]:
        ...
        history = [{
            ...
            'conflicts': 0,
            'time': elapsed,
            'method': 'Enhanced CSP with Slot-Based Constraints'
        }]
